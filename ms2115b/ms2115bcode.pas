...
var
  standardSizes: array[0..8] of TStandardSize;
...
begin
  with standardSizes[0] do begin
    name := '4x3"';
    widthPx := 4 * Screen.PrimaryMonitor.PixelsPerInch;
    heightPx := 3 * Screen.PrimaryMonitor.PixelsPerInch
  end;
  with standardSizes[1] do begin
    name := '5x4"';
    widthPx := 5 * Screen.PrimaryMonitor.PixelsPerInch;
    heightPx := 4 * Screen.PrimaryMonitor.PixelsPerInch
  end;
  with standardSizes[2] do begin
    name := '7x5"';
    widthPx := 7 * Screen.PrimaryMonitor.PixelsPerInch;
    heightPx := 5 * Screen.PrimaryMonitor.PixelsPerInch
  end;
  ...
  with standardSizes[6] do begin
    name := 'ISO A5';
    widthPx := Round(8.3 * Screen.PrimaryMonitor.PixelsPerInch);
    heightPx := Round(5.8 * Screen.PrimaryMonitor.PixelsPerInch)
  end;
  with standardSizes[7] do begin
    name := 'ISO A6';
    widthPx := Round(5.8 * Screen.PrimaryMonitor.PixelsPerInch);
    heightPx := Round(4.1 * Screen.PrimaryMonitor.PixelsPerInch)
  end;
  with standardSizes[8] do begin
    name := 'ISO A7';
    widthPx := Round(4.1 * Screen.PrimaryMonitor.PixelsPerInch);
    heightPx := Round(2.9 * Screen.PrimaryMonitor.PixelsPerInch)
  end;
...
(* signed +ve integers where I want them.                                       *)
...
begin
  Assert(afterShowParam = Ms2115bCodeMagicNumber, 'Internal error: TMs2115bCode bad magic number');
